...
landrun_binary=$(printenv PALOMAR_LANDRUN_BIN || true)
if [ -z "$landrun_binary" ]; then
  echo "error: PALOMAR_LANDRUN_BIN must name the pinned Landrun binary" >&2
  exit 2
fi
landrun_options=()
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    -unrestricted-*|--unrestricted-*)
      echo "error: refusing sandbox-widening Landrun option $1" >&2
      exit 2
      ;;
